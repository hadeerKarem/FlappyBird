...
//		shapeRenderer = new ShapeRenderer();
		...
		maxTubeOffset = Gdx.graphics.getHeight()/2 - gap/2 - 500;
	...
	public void render () {
		...
		if (gameState == 1) {
			...
			//while bird is within the screen or user is touching the screen
			if (birdY > 0) {
				//change birdY
				velocity++;
				birdY -= velocity;
		...
		//game is not active
			...
			if (Gdx.input.justTouched()) {
				//make the game state to be active
				gameState = 1;
			}
		} else if (gameState == 2) {
		...
		//set timer to create time span between each bird frame
		if (pause < 5) {
			pause ++;
		} else {
			pause = 0;
			//changing the state of the bird to make it looks flying
			if (birdState == 0) {
				birdState = 1;
			} else {
				birdState = 0;
		...
		//draw the bird at the center of the screen
		batch.draw(birds[birdState], birdX, birdY);

		font.draw(batch, String.valueOf(score), 100, 200);

		birdCircle.set(Gdx.graphics.getWidth()/2, birdY + birds[birdState].getHeight()/2,
